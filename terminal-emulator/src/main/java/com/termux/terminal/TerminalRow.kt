...
class TerminalRow(columns: Int, style: Long) {
    ...
    fun setChar(columnToSet: Int, codePoint: Int, styleValue: Long) {
        ...
        if (newIsCombining) {
            val combiningCharsCount = WcWidth.zeroWidthCharsCount(text, oldStartOfColumnIndex, oldStartOfColumnIndex + oldCharactersUsedForColumn)
        ...
        if (javaCharDifference > 0) {
            ...
            System.arraycopy(text, oldNextColumnIndex, text, newNextColumnIndex, spaceUsed.toInt() - oldNextColumnIndex)
        }
        ...
        if (oldCodePointDisplayWidth == 2 && newCodePointDisplayWidth == 1) {
            ...
            if (spaceUsed + 1 > text.size) {
                ...
                System.arraycopy(text, newNextColumnIndex, newText, newNextColumnIndex + 1, spaceUsed.toInt() - newNextColumnIndex)
                text = newText
                text = newText
            } else {
                System.arraycopy(text, newNextColumnIndex, text, newNextColumnIndex + 1, spaceUsed.toInt() - newNextColumnIndex)
            }
            ...
            if (col == columns - 1) {
                ...
                val newNextNextColumnIndex = newNextColumnIndex + if (Character.isHighSurrogate(text[newNextColumnIndex])) 2 else 1
                val nextLen = newNextNextColumnIndex - newNextColumnIndex

                // Shift the array leftwards.
                System.arraycopy(text, newNextNextColumnIndex, text, newNextColumnIndex, spaceUsed.toInt() - newNextNextColumnIndex)
