...
class TermuxDocumentsProvider : DocumentsProvider() {
        ...
        while (pending.isNotEmpty() && result.count < MAX_SEARCH_RESULTS) {
            ...
            if (isInsideHome) {
                if (file.isDirectory) {
                    ...
                    if (file.name.lowercase(Locale.getDefault()).contains(query.lowercase(Locale.getDefault()))) {
                        includeFile(result, null, file)
                    }
                }
            }
    ...
    private fun includeFile(result: MatrixCursor, docId: String?, file: File?) {
        ...
        if (documentId == null) {
            documentId = getDocIdForFile(documentFile)
        } else {
            documentFile = getFileForDocId(documentId)
        }

        var flags = 0
        if (documentFile.isDirectory) {
            if (documentFile.canWrite()) flags = flags or Document.FLAG_DIR_SUPPORTS_CREATE
        } else if (documentFile.canWrite()) {
            flags = flags or Document.FLAG_SUPPORTS_WRITE
        }
        if (documentFile.parentFile?.canWrite() == true) flags = flags or Document.FLAG_SUPPORTS_DELETE
        ...
        row.add(Document.COLUMN_SIZE, documentFile.length())
        row.add(Document.COLUMN_MIME_TYPE, mimeType)
        row.add(Document.COLUMN_LAST_MODIFIED, documentFile.lastModified())
