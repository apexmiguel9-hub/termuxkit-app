...
object TermuxInstaller {
    ...
    fun setupBootstrapIfNeeded(activity: Activity, whenDone: Runnable) {
        ...
        var filesDirectoryAccessibleError: Error?
        ...
        if (!isFilesDirectoryAccessible) {
            ...
            if (PackageUtils.isAppInstalledOnExternalStorage(activity) &&
                !TermuxConstants.TERMUX_FILES_DIR_PATH.equals(activity.filesDir.absolutePath.replaceAll("^/data/user/0/", "/data/data/"))) {
        ...
        Thread {
            try {
                ...
                activity.runOnUiThread { whenDone.run() }
    ...
    fun setupStorageSymlinks(context: Context) {
        ...
        Thread {
            try {
                ...
                Logger.logInfo(LOG_TAG, "Setting up storage symlinks at ~/storage/shared, ~/storage/downloads, ~/storage/dcim, ~/storage/pictures, ~/storage/music and ~/storage/movies for directories in \"${Environment.getExternalStorageDirectory().absolutePath}\".")
                ...
                if (dirs != null && dirs.isNotEmpty()) {
                    for (i in dirs.indices) {
                        ...
                        Logger.logInfo(LOG_TAG, "Setting up storage symlinks at ~/storage/$symlinkName for \"${dir.absolutePath}\".")
