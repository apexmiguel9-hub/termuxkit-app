...
class TermuxOpenReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (scheme != null && scheme != UriScheme.SCHEME_FILE) {
    ...
    class ContentProvider : android.content.ContentProvider() {
        ...
        override fun query(
            ...
            val projectionToUse = projection ?: arrayOf(
                MediaStore.MediaColumns.DISPLAY_NAME,
                MediaStore.MediaColumns.SIZE,
                MediaStore.MediaColumns._ID
            )

            val row = arrayOfNulls<Any>(projectionToUse.size)
            for (i in projectionToUse.indices) {
                val column = projectionToUse[i]
            ...
            val cursor = MatrixCursor(projectionToUse)
            cursor.addRow(row)
            return cursor
        }
        ...
        override fun getType(uri: Uri): String? {
            ...
            val extIndex = path?.lastIndexOf('.')?.plus(1) ?: 0
            if (extIndex > 0) {
