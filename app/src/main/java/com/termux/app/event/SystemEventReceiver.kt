...
class SystemEventReceiver : BroadcastReceiver() {
    ...
    private fun onActionBootCompleted(context: Context, intent: Intent) {
        TermuxShellManager.onActionBootCompleted(context, intent)
    }

    private fun onActionPackageUpdated(context: Context, intent: Intent) {
        val data: Uri? = intent.data
        if (data != null && TermuxUtils.isUriDataForTermuxPluginPackage(data)) {
            Logger.logDebug(
                LOG_TAG,
                intent.action?.replace("^android.intent.action.".toRegex(), "") +
    ...
    companion object {
        private var instance: SystemEventReceiver? = null
