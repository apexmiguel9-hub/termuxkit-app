...
class RunCommandService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        if (!RUN_COMMAND_SERVICE.ACTION_RUN_COMMAND.equals(intent.action)) {
        ...
        var executableExtra: String? = IntentUtils.getStringExtraIfSet(intent, RUN_COMMAND_SERVICE.EXTRA_COMMAND_PATH, null)
        ...
        */
        ...
        executionCommand.runner = IntentUtils.getStringExtraIfSet(intent, RUN_COMMAND_SERVICE.EXTRA_RUNNER,
            if (intent.getBooleanExtra(RUN_COMMAND_SERVICE.EXTRA_BACKGROUND, false)) Runner.APP_SHELL.name else Runner.TERMINAL_SESSION.name)
        ...
        if (!executionCommand.workingDirectory.isNullOrEmpty()) execIntent.putExtra(TERMUX_SERVICE.EXTRA_WORKDIR, executionCommand.workingDirectory)
