...
class LocalServerSocket internal constructor(
    ...
    @NonNull
    val mClientSocketListener = Thread(ClientSocketListener())
    ...
    override fun close() {
        ...
        val fd = mLocalSocketRunConfig.getFD() ?: -1
    ...
    fun accept(): LocalClientSocket? {
        ...
        while (true) {
            ...
            val result = LocalSocketManager.accept(mLocalSocketRunConfig.getLogTitle() + " (client)", fd)
            if (result == null || result.retval != 0) {
            ...
            val peerCredResult = LocalSocketManager.getPeerCred(
                mLocalSocketRunConfig.getLogTitle() + " (client)",
                clientFD,
                peerCred
            )
            if (peerCredResult == null || peerCredResult.retval != 0) {
                mLocalSocketManager.onError(
                    LocalSocketErrno.ERRNO_GET_CLIENT_SOCKET_PEER_UID_FAILED.getError(
                        mLocalSocketRunConfig.getTitle(), JniResult.getErrorString(peerCredResult)
                    )
                )
            ...
            if (peerUid != mLocalSocketManager.context.applicationInfo.uid && peerUid != 0) {
                mLocalSocketManager.onDisallowedClientConnected(
                    ...
                    LocalSocketErrno.ERRNO_CLIENT_SOCKET_PEER_UID_DISALLOWED.getError(
                        clientSocket.peerCred.getMinimalString(),
                    ...
                    try {
                        ...
                        mLocalSocketManager.onError(
                            ...
                            LocalSocketErrno.ERRNO_CLIENT_SOCKET_LISTENER_FAILED_WITH_EXCEPTION.getError(
                                t, mLocalSocketRunConfig.getTitle(), t.message
                            )
                        )
                        if (clientSocket != null)
                            clientSocket.closeClientSocket(true)
