...
object TermuxAmSocketServer {
     ...
     */
    @JvmStatic
    fun setupTermuxAmSocketServer(@NonNull context: Context) {
        ...
        if (TermuxAppSharedProperties.getProperties()?.shouldRunTermuxAmSocketServer() == true) {
            ...
            start(context)
            if (termuxAmSocketServer != null && termuxAmSocketServer?.isRunning() == true) {
                enabled = true
    ...
    fun start(@NonNull context: Context) {
        ...
        val amSocketServerRunConfig = AmSocketServerRunConfig(TITLE,
            TermuxConstants.TERMUX_APP.TERMUX_AM_SOCKET_FILE_PATH, TermuxAmSocketServerClient())

        termuxAmSocketServer = AmSocketServer.start(context, amSocketServerRunConfig as LocalSocketRunConfig)
    }
    ...
    fun stop() {
        if (termuxAmSocketServer != null) {
            val error = termuxAmSocketServer?.stop()
            if (error != null) {
    ...
    @JvmStatic
    @Synchronized
    fun updateState(@NonNull context: Context) {
    ...
    @JvmStatic
    @Synchronized
    fun showErrorNotification(@NonNull context: Context, @NonNull error: Error,
        ...
        TermuxPluginUtils.sendPluginCommandErrorNotification(context, LOG_TAG,
            localSocketRunConfig.getTitle() + " Socket Server Error", error.minimalErrorString,
            LocalSocketManager.getErrorMarkdownString(error, localSocketRunConfig, clientSocket))
    }

    @JvmStatic
    fun getTermuxAppAMSocketServerEnabled(@NonNull currentPackageContext: Context): Boolean? {
    ...
    class TermuxAmSocketServerClient : AmSocketServer.AmSocketServerClient() {
        ...
        override fun onError(@NonNull localSocketManager: LocalSocketManager,
            ...
            if (localSocketManager.isRunning()) {
                TermuxAmSocketServer.showErrorNotification(localSocketManager.context, error,
