...
open class TermuxFloatAppSharedPreferences protected constructor(
    ...
    companion object {
        ...
        @JvmStatic
        @Nullable
        fun build(@NonNull context: Context): TermuxFloatAppSharedPreferences? {
            val termuxFloatPackageContext = PackageUtils.getContextForPackage(context, TermuxConstants.TERMUX_FLOAT_PACKAGE_NAME)
            return if (termuxFloatPackageContext == null)
                null
            else
                TermuxFloatAppSharedPreferences(termuxFloatPackageContext)
        }
         ...
         */
        @JvmStatic
        fun build(@NonNull context: Context, exitAppOnError: Boolean): TermuxFloatAppSharedPreferences? {
            val termuxFloatPackageContext = TermuxUtils.getContextForPackageOrExitApp(context, TermuxConstants.TERMUX_FLOAT_PACKAGE_NAME, exitAppOnError)
            return if (termuxFloatPackageContext == null)
                null
            else
                TermuxFloatAppSharedPreferences(termuxFloatPackageContext)
    ...
    fun getFontSize(): Int {
        val fontSize = SharedPreferenceUtils.getInt(mSharedPreferences, TERMUX_FLOAT_APP.KEY_FONTSIZE, DEFAULT_FONTSIZE)
    ...
    fun setLogLevel(context: Context, logLevel: Int, commitToFile: Boolean) {
        val safeLogLevel = Logger.setLogLevel(context, logLevel)
        SharedPreferenceUtils.setInt(mSharedPreferences, TERMUX_FLOAT_APP.KEY_LOG_LEVEL, safeLogLevel, commitToFile)
    }
