...
object TermuxFileUtils {
    ...
    /**
     * Replace "TermuxConstants.TERMUX_PREFIX_DIR_PATH/" or "~/" prefix with termux absolute paths.
     *
    ...
    fun getExpandedTermuxPaths(paths: List<String>?): List<String>? {
        ...
        for (i in paths.indices) {
            expandedPaths.add(getExpandedTermuxPath(paths[i]) ?: "")
        }
    ...
    fun getExpandedTermuxPath(path: String?): String? {
        var safePath = path
        if (safePath != null && safePath.isNotEmpty()) {
            safePath = safePath.replace("^\\TermuxConstants.TERMUX_PREFIX_DIR_PATH", TermuxConstants.TERMUX_PREFIX_DIR_PATH)
            safePath = safePath.replace("^\\TermuxConstants.TERMUX_PREFIX_DIR_PATH/", TermuxConstants.TERMUX_PREFIX_DIR_PATH + "/")
            safePath = safePath.replace("^~/$", TermuxConstants.TERMUX_HOME_DIR_PATH)
            safePath = safePath.replace("^~/", TermuxConstants.TERMUX_HOME_DIR_PATH + "/")
        }
    ...
    /**
     * Replace termux absolute paths with "TermuxConstants.TERMUX_PREFIX_DIR_PATH/" or "~/" prefix.
     *
    ...
    fun getUnExpandedTermuxPaths(paths: List<String>?): List<String>? {
        ...
        for (i in paths.indices) {
            unExpandedPaths.add(getUnExpandedTermuxPath(paths[i]) ?: "")
        }
    ...
    fun getUnExpandedTermuxPath(path: String?): String? {
        var safePath = path
        if (safePath != null && safePath.isNotEmpty()) {
            safePath = safePath.replace("^" + Pattern.quote(TermuxConstants.TERMUX_PREFIX_DIR_PATH) + "/", "\\TermuxConstants.TERMUX_PREFIX_DIR_PATH/")
            safePath = safePath.replace("^" + Pattern.quote(TermuxConstants.TERMUX_HOME_DIR_PATH) + "/", "~/")
        }
     ...
     *                   [getExpandedTermuxPath] before its passed to
    ...
    fun getCanonicalPath(path: String?, prefixForNonAbsolutePath: String?, expandPath: Boolean): String {
        ...
        if (expandPath)
            safePath = getExpandedTermuxPath(safePath)

        return FileUtils.getCanonicalPath(safePath, prefixForNonAbsolutePath)
    }
     ...
     * or validating permissions failed, otherwise `null`.
     */
    fun isTermuxPrefixDirectoryAccessible(createDirectoryIfMissing: Boolean, setMissingPermissions: Boolean): Error? {
     ...
     * or validating permissions failed, otherwise `null`.
     */
    fun isTermuxPrefixStagingDirectoryAccessible(createDirectoryIfMissing: Boolean, setMissingPermissions: Boolean): Error? {
     ...
     * or validating permissions failed, otherwise `null`.
     */
    fun isAppsTermuxAppDirectoryAccessible(createDirectoryIfMissing: Boolean, setMissingPermissions: Boolean): Error? {
    ...
    fun isTermuxPrefixDirectoryEmpty(): Boolean {
        val error = FileUtils.validateDirectoryFileEmptyOrOnlyContainsSpecificFiles("termux prefix",
            TermuxConstants.TERMUX_PREFIX_DIR_PATH, TermuxConstants.TERMUX_PREFIX_DIR_IGNORED_SUB_FILES_PATHS_TO_CONSIDER_AS_EMPTY, true)
        if (error == null)
            return true
     ...
     */
    fun getTermuxFilesStatMarkdownString(@NonNull context: Context): String? {
        val termuxPackageContext = TermuxUtils.getTermuxPackageContext(context)
        ...
        val shellEnv: IShellEnvironment = TermuxShellEnvironment()
        val appShell = AppShell.execute(context, executionCommand, null, shellEnv, null, true)
        ...
        AndroidUtils.appendPropertyToMarkdown(markdownString, "TERMUX_REQUIRED_FILES_DIR_PATH (${TermuxConstants.TERMUX_PREFIX_DIR_PATH})", TermuxConstants.TERMUX_FILES_DIR_PATH)
