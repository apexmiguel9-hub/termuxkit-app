...
import com.termux.shared.shell.command.environment.UnixShellEnvironment.Companion.ENV_TMPDIR
import com.termux.shared.shell.command.environment.UnixShellEnvironment.Companion.ENV_PATH
import com.termux.shared.shell.command.environment.UnixShellEnvironment.Companion.ENV_LD_LIBRARY_PATH
...
open class TermuxShellEnvironment : AndroidShellEnvironment() {
    ...
    companion object {
        ...
        @JvmStatic
        @Synchronized
        fun init(@NonNull currentPackageContext: Context) {
        ...
        @JvmStatic
        @Synchronized
        fun writeEnvironmentToFile(@NonNull currentPackageContext: Context) {
            ...
            val error = FileUtils.writeTextToFile("termux.env.tmp", TermuxConstants.TERMUX_ENV_TEMP_FILE_PATH,
            ...
            val moveError = FileUtils.moveRegularFile("termux.env.tmp", TermuxConstants.TERMUX_ENV_TEMP_FILE_PATH, TermuxConstants.TERMUX_ENV_FILE_PATH, true)
            if (moveError != null) {
                Logger.logErrorExtended(LOG_TAG, moveError.toString())
            }
        }
    }

    init {
        this.shellCommandShellEnvironment = TermuxShellCommandShellEnvironment()
    }

    /** Get shell environment for Termux. */
    @NonNull
    override fun getEnvironment(@NonNull currentPackageContext: Context, isFailSafe: Boolean): HashMap<String, String> {
    ...
    @NonNull
    override fun setupShellCommandArguments(@NonNull executable: String, arguments: Array<String>?): Array<String> {
