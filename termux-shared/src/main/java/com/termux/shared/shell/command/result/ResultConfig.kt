...
class ResultConfig {
     ...
     */
    fun getResultConfigLogString(ignoreNull: Boolean): String {
        val logString = StringBuilder()

        logString.append("Result Pending: `").append(isCommandWithPendingResult()).append("`\n")

        val pendingIntent = resultPendingIntent
        if (pendingIntent != null) {
            logString.append(getResultPendingIntentVariablesLogString(ignoreNull))
            val dirPath = resultDirectoryPath
            if (dirPath != null)
                logString.append("\n")
        }

        val resultDirPath = resultDirectoryPath
        if (resultDirPath != null && resultDirPath.isNotEmpty())
            logString.append(getResultDirectoryVariablesLogString(ignoreNull))
    ...
    fun getResultPendingIntentVariablesLogString(ignoreNull: Boolean): String {
        val pendingIntent = resultPendingIntent
        if (pendingIntent == null) return "Result PendingIntent Creator: -"
        ...
        resultPendingIntentVariablesString.append("Result PendingIntent Creator: `")
            .append(pendingIntent.creatorPackage).append("`")
     ...
     */
    fun getResultConfigMarkdownString(): String {
        ...
        val dirPath = resultDirectoryPath
        if (dirPath != null) {
            markdownString.append("\n").append(MarkdownUtils.getSingleLineMarkdownStringEntry("Result Directory Path", dirPath, "-"))
            markdownString.append("\n").append(MarkdownUtils.getSingleLineMarkdownStringEntry("Result Single File", resultSingleFile, "-"))
            markdownString.append("\n").append(MarkdownUtils.getSingleLineMarkdownStringEntry("Result File Basename", resultFileBasename, "-"))
            markdownString.append("\n").append(MarkdownUtils.getSingleLineMarkdownStringEntry("Result File Output Format", resultFileOutputFormat, "-"))
            markdownString.append("\n").append(MarkdownUtils.getSingleLineMarkdownStringEntry("Result File Error Format", resultFileErrorFormat, "-"))
            markdownString.append("\n").append(MarkdownUtils.getSingleLineMarkdownStringEntry("Result Files Suffix", resultFilesSuffix, "-"))
        }
    ...
    companion object {
        ...
        fun getResultConfigLogString(resultConfig: ResultConfig?, ignoreNull: Boolean): String {
            return resultConfig?.getResultConfigLogString(ignoreNull) ?: "null"
        }

        @JvmStatic
        fun getResultConfigMarkdownString(resultConfig: ResultConfig?): String {
            return resultConfig?.getResultConfigMarkdownString() ?: "null"
        }
    }
}
