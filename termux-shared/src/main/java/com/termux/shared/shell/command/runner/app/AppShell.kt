 ...
 */
    ...
    ): AppShell? = start(currentPackageContext, executionCommand, appShellClient, shellEnvironmentClient, additionalEnvironment, isSynchronous)
    ...
    fun start(
        ...
        val environmentList = ShellEnvironmentUtils.convertEnvironmentToEnviron(environment).toList()
    ...
    private fun executeInner(@NonNull context: Context) {
        mExecutionCommand.pid = ShellUtils.getPid(process)

        Logger.logDebug(LOG_TAG, "Running \"${mExecutionCommand.getCommandIdAndLabelLogString()}\" AppShell with pid ${mExecutionCommand.pid}")
        ...
        val STDOUT = StreamGobbler("${mExecutionCommand.pid}-stdout", process.inputStream, mExecutionCommand.resultData.stdout, mExecutionCommand.backgroundCustomLogLevel)
        val STDERR = StreamGobbler("${mExecutionCommand.pid}-stderr", process.errorStream, mExecutionCommand.resultData.stderr, mExecutionCommand.backgroundCustomLogLevel)
        ...
        if (!DataUtils.isNullOrEmpty(mExecutionCommand.stdin)) {
            try {
                ...
                if (message != null && (message.contains("EPIPE") || message.contains("Stream closed"))) {
                    ...
                    // returning null
                    mExecutionCommand.setStateFailed(Errno.ERRNO_FAILED.code, context.getString(R.string.error_exception_received_while_executing_app_shell_command, mExecutionCommand.getCommandIdAndLabelLogString(), e.message), e)
        ...
        // Process result
        if (exitCode == 0)
            Logger.logDebug(LOG_TAG, "The \"${mExecutionCommand.getCommandIdAndLabelLogString()}\" AppShell with pid ${mExecutionCommand.pid} exited normally")
        else
            Logger.logDebug(LOG_TAG, "The \"${mExecutionCommand.getCommandIdAndLabelLogString()}\" AppShell with pid ${mExecutionCommand.pid} exited with code: $exitCode")
    ...
    fun getExecutionCommand(): ExecutionCommand = mExecutionCommand
    ...
    companion object {
        ...
        fun execute(
            ...
            @Nullable additionalEnvironment: HashMap<String, String>?,
            isSynchronous: Boolean
        ): AppShell? {
            val executable = executionCommand.executable
            if (executable == null || executable.isEmpty()) {
                executionCommand.setStateFailed(Errno.ERRNO_FAILED.code,
                    currentPackageContext.getString(R.string.error_executable_unset, executionCommand.getCommandIdAndLabelLogString()))
                processAppShellResult(null, executionCommand)
                return null
            }

            var workingDir = executionCommand.workingDirectory
            if (workingDir == null || workingDir.isEmpty())
                workingDir = shellEnvironmentClient.getDefaultWorkingDirectoryPath()
            if (workingDir.isEmpty())
                workingDir = "/"
            executionCommand.workingDirectory = workingDir
    ...
    @JvmStatic
    fun processAppShellResult(appShell: AppShell?, executionCommand: ExecutionCommand?) {
            var finalExecutionCommand = executionCommand
            if (appShell != null)
                finalExecutionCommand = appShell.mExecutionCommand

            if (finalExecutionCommand == null) return

            if (finalExecutionCommand.shouldNotProcessResults()) {
                Logger.logDebug(LOG_TAG, "Ignoring duplicate call to process \"${finalExecutionCommand.getCommandIdAndLabelLogString()}\" AppShell result")
                return
            }

            Logger.logDebug(LOG_TAG, "Processing \"${finalExecutionCommand.getCommandIdAndLabelLogString()}\" AppShell result")
            ...
            if (appShell != null && appShell.mAppShellClient != null) {
                ...
                if (!finalExecutionCommand.isStateFailed())
                    finalExecutionCommand.setState(ExecutionState.SUCCESS)
            }
        }
    }
}
