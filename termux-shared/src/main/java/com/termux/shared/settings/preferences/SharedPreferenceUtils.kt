...
object SharedPreferenceUtils {
    ...
    fun getStringSet(sharedPreferences: SharedPreferences?, key: String, def: Set<String>?): Set<String> {
        if (sharedPreferences == null) {
            Logger.logError(LOG_TAG, "Error getting Set<String> value for the \"$key\" key from null shared preferences. Returning default value.")
            return def?.toMutableSet() ?: mutableSetOf()
        }

        return try {
            val result = sharedPreferences.getStringSet(key, def)
            result?.toMutableSet() ?: def?.toMutableSet() ?: mutableSetOf()
        } catch (e: ClassCastException) {
            Logger.logStackTraceWithMessage(LOG_TAG, "Error getting Set<String> value for the \"$key\" key from shared preferences. Returning default value.", e)
            def?.toMutableSet() ?: mutableSetOf()
        }
    }
