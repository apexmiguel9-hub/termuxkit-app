...
object Logger {
    ...
    fun logMessage(logPriority: Int, tag: String, message: String) {
        ...
        when {
            logPriority == Log.ERROR && CURRENT_LOG_LEVEL >= LOG_LEVEL_NORMAL -> Log.e(fullTag, message)
            logPriority == Log.WARN && CURRENT_LOG_LEVEL >= LOG_LEVEL_NORMAL -> Log.w(fullTag, message)
            logPriority == Log.INFO && CURRENT_LOG_LEVEL >= LOG_LEVEL_NORMAL -> Log.i(fullTag, message)
            logPriority == Log.DEBUG && CURRENT_LOG_LEVEL >= LOG_LEVEL_DEBUG -> Log.d(fullTag, message)
            logPriority == Log.VERBOSE && CURRENT_LOG_LEVEL >= LOG_LEVEL_VERBOSE -> Log.v(fullTag, message)
        }
    }
    ...
    fun logExtendedMessage(logLevel: Int, tag: String, message: String?) {
        ...
        while (remainingMessage.isNotEmpty()) {
            if (remainingMessage.isNotEmpty()) {
                var cutOffIndex = if (maxEntrySize > remainingMessage.length) remainingMessage.length else maxEntrySize
                val nextNewlineIndex = remainingMessage.lastIndexOf('\n', cutOffIndex - 1)
                if (nextNewlineIndex != -1) {
                    cutOffIndex = nextNewlineIndex + 1
                }
                messagesList.add(remainingMessage.substring(0, cutOffIndex))
                remainingMessage = remainingMessage.substring(cutOffIndex)
            } else {
                messagesList.add(remainingMessage)
                break
            }
        }

        messagesList.forEachIndexed { i, msg ->
            val prefix = if (messagesList.size > 1) "(${i + 1}/${messagesList.size})\n" else ""
            logMessage(logLevel, tag, prefix + msg)
        }
    }
    ...
    fun getMessageAndStackTracesString(message: String?, throwablesList: List<Throwable>?): String? {
        return when {
            message == null && (throwablesList == null || throwablesList.isEmpty()) -> null
    ...
    fun getStackTracesStringArray(throwable: Throwable): Array<String?> =
        getStackTracesStringArray(listOf(throwable))

    fun getStackTracesStringArray(throwablesList: List<Throwable>?): Array<String?> {
        throwablesList ?: return arrayOfNulls(0)
        return Array(throwablesList.size) { i -> getStackTraceString(throwablesList[i]) }
    }

    fun getStackTracesString(label: String?, stackTraceStringArray: Array<String?>?): String {
        val safeLabel = label ?: "StackTraces:"
        val stackTracesString = StringBuilder(safeLabel)

        if (stackTraceStringArray.isNullOrEmpty()) {
            stackTracesString.append(" -")
        } else {
            stackTraceStringArray.forEachIndexed { i, trace ->
    ...
    fun getSingleLineLogStringEntry(label: String, obj: Any?, def: String): String =
        if (obj != null) "$label: `$obj`" else "$label: $def"
    ...
    fun setLogLevel(context: Context?, logLevel: Int): Int {
        ...
        context?.let {
            showToast(it, getLogLevelLabel(it, CURRENT_LOG_LEVEL, false), true)
        }
    ...
    fun getFullTag(tag: String): String =
        if (DEFAULT_LOG_TAG == tag) tag else "$DEFAULT_LOG_TAG.$tag"

    fun isLogLevelValid(logLevel: Int?): Boolean = logLevel != null && logLevel in LOG_LEVEL_OFF..MAX_LOG_LEVEL
    ...
    fun shouldEnableLoggingForCustomLogLevel(customLogLevel: Int?): Boolean {
        ...
        val safeLevel = if (isLogLevelValid(customLogLevel)) customLogLevel else LOG_LEVEL_VERBOSE
        return safeLevel >= CURRENT_LOG_LEVEL
    }
