...
object FileUtils {
    ...
    @JvmField
    val APP_EXECUTABLE_FILE_PERMISSIONS = "r-x" // Default: "r-x"
    ...
    @JvmField
    val APP_WORKING_DIRECTORY_PERMISSIONS = "rwx" // Default: "rwx"
     ...
     * If path is not an absolute path and `prefixForNonAbsolutePath` is not `null`, then
     * `prefixForNonAbsolutePath` + "/" is prefixed before path before getting canonical path.
     * If path is not an absolute path and `prefixForNonAbsolutePath` is `null`, then
    ...
    fun getCanonicalPath(path: String?, prefixForNonAbsolutePath: String?): String {
        val absolutePath = path.orEmpty()

        // If path is already an absolute path
        if (!absolutePath.startsWith("/")) {
            val finalPath = if (prefixForNonAbsolutePath != null)
                "$prefixForNonAbsolutePath/$absolutePath"
            else
                "/$absolutePath"
            return File(finalPath).canonicalPath
        }

        return runCatching { File(absolutePath).canonicalPath }.getOrElse { absolutePath }
    }
     ...
     *                            converted.
     * @param toLower If set to `true`, then file name will be converted to lower case.
     * @return Returns the `sanitized name`.
     */
    fun sanitizeFileName(fileName: String?, sanitizeWhitespaces: Boolean, toLower: Boolean): String? {
        return fileName?.let {
            val sanitizedFileName = if (sanitizeWhitespaces)
                it.replace("[\\\\/:*?\"<>| \t\n]".toRegex(), "_")
            else
                it.replace("[\\\\/:*?\"<>|]".toRegex(), "_")

            if (toLower) sanitizedFileName.lowercase() else sanitizedFileName
        }
    }
    ...
    fun isPathInDirPath(path: String?, dirPath: String?, ensureUnder: Boolean): Boolean {
        return isPathInDirPaths(path, listOfNotNull(dirPath), ensureUnder)
    }
    ...
    fun validateDirectoryFileEmptyOrOnlyContainsSpecificFiles(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "isDirectoryFileEmptyOrOnlyContainsSpecificFiles")
        ...
        return runCatching {
            ...
            if (fileType != FileType.NO_EXIST && fileType != FileType.DIRECTORY) {
                return FileUtilsErrno.ERRNO_NON_DIRECTORY_FILE_FOUND.getError("${actualLabel}directory", filePath).setLabel("${actualLabel}directory")
            }
            ...
            if (fileType == FileType.NO_EXIST) {
                return if (ignoreNonExistentFile)
                    ...
                    val newLabel = "${actualLabel}directory to check if is empty or only contains specific files"
                    FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel)
                }
            }
            ...
            if (ignoredSubFilePaths.isNullOrEmpty())
                return FileUtilsErrno.ERRNO_NON_EMPTY_DIRECTORY_FILE.getError(actualLabel, filePath)
            ...
            FileUtilsErrno.ERRNO_VALIDATE_DIRECTORY_EMPTY_OR_ONLY_CONTAINS_SPECIFIC_FILES_FAILED_WITH_EXCEPTION.getError(e, "${actualLabel}directory", filePath, e.message)
        }
    }
    ...
    fun nonIgnoredSubFileExists(subFiles: Array<File>?, ignoredSubFilePaths: List<String>): Boolean {
        ...
        for (subFile in subFiles) {
            ...
            if (subFilePath !in ignoredSubFilePaths) {
     ...
     *                       finding if file exists. Check [getFileType] for details.
     ...
     * @param followLinks The `boolean` that decides if symlinks will be followed while
     *                       finding if file exists. Check [getFileType] for details.
     * @return Returns `true` if regular or directory file exists, otherwise `false`.
     */
     ...
     *
     * This function is a wrapper for [FileTypes.getFileType]
     *
     * @param filePath The `path` for file to check.
     * @param followLinks The `boolean` that decides if symlinks will be followed while
    ...
    fun validateRegularFileExistenceAndPermissions(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}regular file path", "validateRegularFileExistenceAndPermissions")
        ...
        return runCatching {
            ...
            if (fileType != FileType.NO_EXIST && fileType != FileType.REGULAR) {
                return FileUtilsErrno.ERRNO_NON_REGULAR_FILE_FOUND.getError(actualLabel.ifEmpty { "file" }, filePath)?.setLabel(actualLabel.ifEmpty { "file" })
            }
            ...
            if (setPermissions && permissionsToCheck != null && fileType == FileType.REGULAR) {
                ...
                if (parentDirPath == null || (isPathUnderParentDirPath && getFileType(parentDirPath, false) == FileType.DIRECTORY)) {
                    if (setMissingPermissionsOnly)
                        setMissingFilePermissions("${actualLabel}file", filePath, permissionsToCheck)
                    else
                        setFilePermissions("${actualLabel}file", filePath, permissionsToCheck)
                }
            }
            ...
            if (fileType != FileType.REGULAR) {
                val newLabel = "${actualLabel}regular file"
                return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel)
            }
            ...
            if (parentDirPath == null || !isPathUnderParentDirPath || !ignoreErrorsIfPathIsUnderParentDirPath) {
                if (permissionsToCheck != null) {
                    ...
                    return checkMissingFilePermissions("${actualLabel}regular", filePath, permissionsToCheck, false)
                }
            }
    ...
    fun validateDirectoryFileExistenceAndPermissions(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}directory file path", "validateDirectoryExistenceAndPermissions")
        ...
        return runCatching {
            ...
            if (createDirectoryIfMissing || setPermissions) {
                ...
                if (parentDirPath == null || (isPathInParentDirPath && getFileType(parentDirPath, false) == FileType.DIRECTORY)) {
                    ...
                    if (createDirectoryIfMissing && fileType == FileType.NO_EXIST) {
                        Logger.logVerbose(LOG_TAG, "Creating $actualLabel directory file at path \"$filePath\"")
                        ...
                        val newFileType = getFileType(filePath, false)
                        if (!result && newFileType != FileType.DIRECTORY)
                            return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED.getError("${actualLabel}directory file", filePath)
                    }
                    ...
                    if (setPermissions && permissionsToCheck != null && fileType == FileType.DIRECTORY) {
                        if (setMissingPermissionsOnly)
                            setMissingFilePermissions("${actualLabel}directory", filePath, permissionsToCheck)
                        else
                            setFilePermissions("${actualLabel}directory", filePath, permissionsToCheck)
                    }
                }
            }
            ...
            if (parentDirPath == null || !isPathInParentDirPath || !ignoreErrorsIfPathIsInParentDirPath) {
                ...
                if (fileType != FileType.DIRECTORY) {
                    val newLabel = "${actualLabel}directory"
                    return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel)
                }

                if (permissionsToCheck != null) {
                    // Check if permissions are missing
                    return checkMissingFilePermissions("${actualLabel}directory", filePath, permissionsToCheck, ignoreIfNotExecutable)
                }
            }
     ...
     *
     * This function is a wrapper for [validateDirectoryFileExistenceAndPermissions].
     *
     ...
     * Create a regular file at path.
     *
     * This function is a wrapper for [validateRegularFileExistenceAndPermissions].
     *
     * @param label The optional label for the regular file. This can optionally be `null`.
     * @param filePath The `path` for regular file to create.
     * @param permissionsToCheck The 3 character string that contains the "r", "w", "x" or "-" in-order.
    ...
    fun createRegularFile(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "createRegularFile")
        ...
        val error = createParentDirectoryFile("${actualLabel}regular file parent", filePath)
        if (error != null)
            return error

        val createResult = runCatching {
            Logger.logVerbose(LOG_TAG, "Creating ${actualLabel}regular file at path \"$filePath\"")

            if (!file.createNewFile())
                return FileUtilsErrno.ERRNO_CREATING_FILE_FAILED.getError("${actualLabel}regular file", filePath)

            null
        }.getOrElse { e ->
            FileUtilsErrno.ERRNO_CREATING_FILE_FAILED_WITH_EXCEPTION.getError(e, "${actualLabel}regular file", filePath, e.message)
        }

        // If file creation was successful, validate permissions
     ...
     *
     * This function is a wrapper for [createSymlinkFile].
     *
    ...
    fun createSymlinkFile(
        ...
        if (targetFilePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}target file path", "createSymlinkFile")
        if (destFilePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}destination file path", "createSymlinkFile")
        ...
        return runCatching {
            ...
            if (targetFileType == FileType.NO_EXIST) {
                ...
                if (!allowDangling) {
                    val newLabel = "${actualLabel}symlink target file"
                    return FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, targetFileAbsolutePath).setLabel(newLabel)
                }
            }
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                if (overwriteOnlyIfDestIsASymlink && destFileType != FileType.SYMLINK)
                    return FileUtilsErrno.ERRNO_CANNOT_OVERWRITE_A_NON_SYMLINK_FILE_TYPE.getError("${actualLabel} file", destFilePath, targetFilePath, destFileType.name)

                // Delete the destination file
                val error = deleteFile("${actualLabel}symlink destination", destFilePath, true)
                if (error != null)
                    return error
            } else {
                // Create the destination file parent directory
                val error = createParentDirectoryFile("${actualLabel}symlink destination file parent", destFilePath)
                if (error != null)
                    return error
            }

            // create a symlink at destFilePath to targetFilePath
            Logger.logVerbose(LOG_TAG, "Creating ${actualLabel}symlink file at path \"$destFilePath\" to \"$targetFilePath\"")
     ...
     *
     * This function is a wrapper for [copyOrMoveFile].
     *
     ...
     * @param moveFile If set to `true`, then source file will be moved, otherwise it will be
     *                 copied.
     * @param ignoreNonExistentSrcFile The `boolean` that decides if it should be considered an
    ...
    fun copyOrMoveFile(
        ...
        if (srcFilePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}source file path", "copyOrMoveFile")
        if (destFilePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}destination file path", "copyOrMoveFile")

        val mode = if (moveFile) "Moving" else "Copying"
        val modePast = if (moveFile) "moved" else "copied"

        return runCatching {
            Logger.logVerbose(LOG_TAG, "$mode $actualLabel source file from \"$srcFilePath\" to destination \"$destFilePath\"")
            ...
            if (srcFileType == FileType.NO_EXIST) {
                ...
                return if (ignoreNonExistentSrcFile)
                    ...
                    val newLabel = "${actualLabel}source file"
                    FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, srcFilePath).setLabel(newLabel)
                }
            }
            ...
            if ((allowedFileTypeFlags and srcFileType.value) <= 0)
                return FileUtilsErrno.ERRNO_FILE_NOT_AN_ALLOWED_FILE_TYPE.getError("${actualLabel}source file meant to be $modePast", srcFilePath, FileTypes.convertFileTypeFlagsToNamesString(allowedFileTypeFlags))

            // If source and destination file path are the same
            if (srcFileCanonicalPath == destFileCanonicalPath)
                return FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_TO_SAME_PATH.getError("$mode $actualLabel source file", srcFilePath, destFilePath)
            ...
            if (destFileType != FileType.NO_EXIST) {
                ...
                if (overwriteOnlyIfDestSameFileTypeAsSrc && destFileType != srcFileType)
                    return FileUtilsErrno.ERRNO_CANNOT_OVERWRITE_A_DIFFERENT_FILE_TYPE.getError("${actualLabel}source file", mode.lowercase(), srcFilePath, destFilePath, destFileType.name, srcFileType.name)

                // Delete the destination file
                val error = deleteFile("${actualLabel}destination", destFilePath, true)
                if (error != null)
                    return error
            }
            ...
            if (moveFile) {
                ...
                if (!srcFile.renameTo(destFile)) {
                    ...
                    if (srcFileType == FileType.DIRECTORY && destFileCanonicalPath.startsWith("$srcFileCanonicalPath${File.separator}"))
                        return FileUtilsErrno.ERRNO_CANNOT_MOVE_DIRECTORY_TO_SUB_DIRECTORY_OF_ITSELF.getError("${actualLabel}source directory", srcFilePath, destFilePath)

                    // If rename failed, then we copy
                    Logger.logVerbose(LOG_TAG, "Renaming $actualLabel source file to destination failed, attempting to copy.")
                    copyFile = true
                }
            }
            ...
            if (copyFile) {
                ...
                val error = createParentDirectoryFile("${actualLabel}dest file parent", destFilePath)
                if (error != null)
                    return error
                ...
                when (srcFileType) {
                    ...
                    FileType.SYMLINK -> {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                            ...
                            val symlinkError = createSymlinkFile("${actualLabel}dest", Os.readlink(srcFilePath), destFilePath)
            ...
            if (moveFile) {
                ...
                val error = deleteFile("${actualLabel}source", srcFilePath, true)
                if (error != null)
                    return error
            }
            ...
            FileUtilsErrno.ERRNO_COPYING_OR_MOVING_FILE_FAILED_WITH_EXCEPTION.getError(e, "$mode $actualLabel file", srcFilePath, destFilePath, e.message)
        }
    }
    ...
    fun deleteFile(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "deleteFile")

        return runCatching {
            val file = File(filePath)
            val fileType = getFileType(filePath, false)

            Logger.logVerbose(LOG_TAG, "Processing delete of $actualLabel file at path \"$filePath\" of type \"${fileType.name}\"")
            ...
            if (fileType == FileType.NO_EXIST) {
                ...
                return if (ignoreNonExistentFile)
                    ...
                    // Else return with error
                    val newLabel = "${actualLabel}file meant to be deleted"
                    FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel)
                }
            }
            ...
            if ((allowedFileTypeFlags and fileType.value) <= 0) {
                ...
                if (ignoreWrongFileType) {
                    Logger.logVerbose(LOG_TAG, "Ignoring deletion of $actualLabel file at path \"$filePath\" of type \"${fileType.name}\" not matching allowed file types: ${FileTypes.convertFileTypeFlagsToNamesString(allowedFileTypeFlags)}")
                    return null
                }

                // Else return with error
                return FileUtilsErrno.ERRNO_FILE_NOT_AN_ALLOWED_FILE_TYPE.getError("${actualLabel}file meant to be deleted", filePath, fileType.name, FileTypes.convertFileTypeFlagsToNamesString(allowedFileTypeFlags))
            }

            Logger.logVerbose(LOG_TAG, "Deleting $actualLabel file at path \"$filePath\"")

            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                /*
                 * Try to use SecureDirectoryStream if available for safer directory
            ...
            val newFileType = getFileType(filePath, false)
            if (newFileType != FileType.NO_EXIST)
                return FileUtilsErrno.ERRNO_FILE_STILL_EXISTS_AFTER_DELETING.getError("${actualLabel}file meant to be deleted", filePath)
     ...
     *
     * This function is a wrapper for [clearDirectory].
     *
    ...
    fun clearDirectory(label: String?, filePath: String?): Error? {
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "clearDirectory")

        return runCatching {
            Logger.logVerbose(LOG_TAG, "Clearing $actualLabel directory at path \"$filePath\"")
    ...
    fun deleteFilesOlderThanXDays(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (days < 0)
            return FunctionErrno.ERRNO_INVALID_PARAMETER.getError("${actualLabel}days", "deleteFilesOlderThanXDays", " It must be >= 0.")

        return runCatching {
            Logger.logVerbose(LOG_TAG, "Deleting files under $actualLabel directory at path \"$filePath\" older than $days days")
            ...
            if (fileType == FileType.NO_EXIST) {
                ...
                return if (ignoreNonExistentFile)
                    ...
                    // Else return with error
                    val newLabel = "${actualLabel}directory under which files had to be deleted"
                    FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel)
                }
            }
            ...
            while (filesToDelete.hasNext()) {
                ...
                val error = deleteFile("${actualLabel} directory sub", subFile.absolutePath, true, true, allowedFileTypeFlags)
                if (error != null)
                    return error
            }
    ...
    fun readTextFromFile(
        ...
        filePath: String?,
        charset: Charset?,
        @NonNull dataStringBuilder: StringBuilder,
        ignoreNonExistentFile: Boolean
    ): Error? {
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "readStringFromFile")

        Logger.logVerbose(LOG_TAG, "Reading text from $actualLabel file at path \"$filePath\"")
        ...
        if (fileType == FileType.NO_EXIST) {
            return if (ignoreNonExistentFile)
                ...
                val newLabel = "${actualLabel}file meant to be read"
                FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel)
            }
        }

        val actualCharset = charset ?: Charset.defaultCharset()

        // Check if charset is supported
        val error = isCharsetSupported(actualCharset)
        if (error != null)
            return error
        ...
        try {
            ...
            bufferedReader = BufferedReader(InputStreamReader(fileInputStream, actualCharset))
            ...
            return FileUtilsErrno.ERRNO_READING_TEXT_FROM_FILE_FAILED_WITH_EXCEPTION.getError(e, "${label}file", filePath, e.message)
        } finally {
    ...
    fun <T : Serializable> readSerializableObjectFromFile(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return ReadSerializableObjectResult(FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "readSerializableObjectFromFile"), null)

        Logger.logVerbose(LOG_TAG, "Reading serializable object from $actualLabel file at path \"$filePath\"")

        val fileType = getFileType(filePath, false)

        // If file exists but not a regular file
        if (fileType != FileType.NO_EXIST && fileType != FileType.REGULAR) {
            return ReadSerializableObjectResult(FileUtilsErrno.ERRNO_NON_REGULAR_FILE_FOUND.getError(actualLabel.ifEmpty { "file" }, filePath)?.setLabel(actualLabel.ifEmpty { "file" }), null)
        }
        ...
        if (fileType == FileType.NO_EXIST) {
            return if (ignoreNonExistentFile)
                ...
                val newLabel = "${actualLabel}file meant to be read"
                ReadSerializableObjectResult(FileUtilsErrno.ERRNO_FILE_NOT_FOUND_AT_PATH.getError(newLabel, filePath).setLabel(newLabel), null)
            }
        }
        ...
        try {
            ...
            return ReadSerializableObjectResult(FileUtilsErrno.ERRNO_READING_SERIALIZABLE_OBJECT_TO_FILE_FAILED_WITH_EXCEPTION.getError(e, "${label}file", filePath, e.message), null)
        } finally {
            closeCloseable(fileInputStream)
    ...
    fun writeTextToFile(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "writeStringToFile")

        Logger.logVerbose(LOG_TAG, Logger.getMultiLineLogStringEntry("Writing text to $actualLabel file at path \"$filePath\"", DataUtils.getTruncatedCommandOutput(dataString, Logger.LOGGER_ENTRY_MAX_SAFE_PAYLOAD, true, false, true), "-"))

        val error = preWriteToFile(actualLabel, filePath)
        if (error != null)
            return error
        ...
        // Check if charset is supported
        val charsetError = isCharsetSupported(actualCharset)
        ...
        try {
            ...
            bufferedWriter = BufferedWriter(OutputStreamWriter(fileOutputStream, actualCharset))

            bufferedWriter.write(dataString)
            bufferedWriter.flush()
        } catch (e: Exception) {
            return FileUtilsErrno.ERRNO_WRITING_TEXT_TO_FILE_FAILED_WITH_EXCEPTION.getError(e, "${label}file", filePath, e.message)
        } finally {
    ...
    fun <T : Serializable> writeSerializableObjectToFile(
        ...
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "writeSerializableObjectToFile")

        Logger.logVerbose(LOG_TAG, "Writing serializable object to $actualLabel file at path \"$filePath\"")
        ...
        try {
            ...
            return FileUtilsErrno.ERRNO_WRITING_SERIALIZABLE_OBJECT_TO_FILE_FAILED_WITH_EXCEPTION.getError(e, "${label}file", filePath, e.message)
        } finally {
            closeCloseable(fileOutputStream)
    ...
    fun setFilePermissions(label: String?, filePath: String?, permissionsToSet: String?) {
        ...
        if (permissionsToSet?.contains("r") == true) {
            if (!file.canRead()) {
                Logger.logVerbose(LOG_TAG, "Setting read permissions for $actualLabel file at path \"$filePath\"")
                file.setReadable(true)
            }
        } else {
            if (file.canRead()) {
                Logger.logVerbose(LOG_TAG, "Removing read permissions for $actualLabel file at path \"$filePath\"")
                file.setReadable(false)
            }
        }

        if (permissionsToSet?.contains("w") == true) {
            if (!file.canWrite()) {
                Logger.logVerbose(LOG_TAG, "Setting write permissions for $actualLabel file at path \"$filePath\"")
                file.setWritable(true)
            }
        } else {
            if (file.canWrite()) {
                Logger.logVerbose(LOG_TAG, "Removing write permissions for $actualLabel file at path \"$filePath\"")
                file.setWritable(false)
            }
        }

        if (permissionsToSet?.contains("x") == true) {
            if (!file.canExecute()) {
                Logger.logVerbose(LOG_TAG, "Setting execute permissions for $actualLabel file at path \"$filePath\"")
                file.setExecutable(true)
            }
        } else {
            if (file.canExecute()) {
                Logger.logVerbose(LOG_TAG, "Removing execute permissions for $actualLabel file at path \"$filePath\"")
    ...
    fun setMissingFilePermissions(label: String?, filePath: String?, permissionsToSet: String?) {
        ...
        if (permissionsToSet?.contains("r") == true && !file.canRead()) {
            Logger.logVerbose(LOG_TAG, "Setting missing read permissions for $actualLabel file at path \"$filePath\"")
            file.setReadable(true)
        }

        if (permissionsToSet?.contains("w") == true && !file.canWrite()) {
            Logger.logVerbose(LOG_TAG, "Setting missing write permissions for $actualLabel file at path \"$filePath\"")
            file.setWritable(true)
        }

        if (permissionsToSet?.contains("x") == true && !file.canExecute()) {
            Logger.logVerbose(LOG_TAG, "Setting missing execute permissions for $actualLabel file at path \"$filePath\"")
            file.setExecutable(true)
        }
    }
    ...
    fun checkMissingFilePermissions(label: String?, filePath: String?, permissionsToCheck: String?, ignoreIfNotExecutable: Boolean): Error? {
        val actualLabel = label.takeUnless { it.isNullOrEmpty() }?.let { "$it " }.orEmpty()
        if (filePath.isNullOrEmpty())
            return FunctionErrno.ERRNO_NULL_OR_EMPTY_PARAMETER.getError("${actualLabel}file path", "checkMissingFilePermissions")
        ...
        if (permissionsToCheck?.contains("r") == true && !file.canRead()) {
            return FileUtilsErrno.ERRNO_FILE_NOT_READABLE.getError(actualLabel.ifEmpty { "file" }, filePath)?.setLabel(actualLabel.ifEmpty { "file" })
        }

        // If file is not writable
        if (permissionsToCheck?.contains("w") == true && !file.canWrite()) {
            return FileUtilsErrno.ERRNO_FILE_NOT_WRITABLE.getError(actualLabel.ifEmpty { "file" }, filePath)?.setLabel(actualLabel.ifEmpty { "file" })
        }
        // If file is not executable
        // This canExecute() will give "avc: granted { execute }" warnings for target sdk 29
        else if (permissionsToCheck?.contains("x") == true && !file.canExecute() && !ignoreIfNotExecutable) {
            return FileUtilsErrno.ERRNO_FILE_NOT_EXECUTABLE.getError(actualLabel.ifEmpty { "file" }, filePath)?.setLabel(actualLabel.ifEmpty { "file" })
        }
     ...
     *
     * @param error The original [Error] returned by one of the [FileUtils] functions.
     * @return Returns the shorter [Error] if one exists, otherwise original `error`.
     */
    fun getShortFileUtilsError(error: Error): Error {
        val type = error.type
        if (FileUtilsErrno.TYPE != type) return error

        val shortErrno = FileUtilsErrno.ERRNO_SHORT_MAPPING[Errno.valueOf(type, error.code)] ?: return error

        val throwables = error.throwablesList
        return if (throwables.isEmpty())
            shortErrno.getError(DataUtils.getDefaultIfNull(error.label, "file"))
        else
            shortErrno.getError(error.throwablesList, DataUtils.getDefaultIfNull(error.label, "file"))
    }
     ...
     * @return Returns the file dirname if not `null`.
     */
    fun getFileDirname(filePath: String?): String? {
        return filePath?.let { safeFilePath ->
            val lastSlash = safeFilePath.lastIndexOf('/')
            if (lastSlash == -1) null else safeFilePath.substring(0, lastSlash)
        }
    }
    ...
    fun getFileBasename(filePath: String?): String? {
        return filePath?.let { safeFilePath ->
            val lastSlash = safeFilePath.lastIndexOf('/')
            if (lastSlash == -1) safeFilePath else safeFilePath.substring(lastSlash + 1)
        }
    }
     ...
     *
     * @param filePath The `path` for file.
     * @return Returns the file basename without extension if not `null`.
     */
    fun getFileBasenameWithoutExtension(filePath: String?): String? {
        return getFileBasename(filePath)?.let { fileBasename ->
            val lastDot = fileBasename.lastIndexOf('.')
            if (lastDot == -1) fileBasename else fileBasename.substring(0, lastDot)
        }
    }
}
