...
import com.termux.shared.file.filesystem.NativeDispatcher.fstat
import com.termux.shared.file.filesystem.NativeDispatcher.lstat
import com.termux.shared.file.filesystem.NativeDispatcher.stat
import java.io.File
...
class FileAttributes private constructor(
    ...
    companion object {
        ...
        fun get(filePath: String?, followLinks: Boolean): FileAttributes {
            val fileAttributes = if (filePath == null || filePath.isEmpty()) {
    ...
    fun file(): String? {
        return filePath ?: fileDescriptor?.toString()
    }
    ...
    fun fileKey(): FileKey {
        return key ?: synchronized(this) {
            key ?: FileKey(st_dev, st_ino).also { key = it }
        }
    }

    fun owner(): String {
        return owner ?: synchronized(this) {
            owner ?: st_uid.toString().also { owner = it }
        } ?: ""
    }

    fun group(): String {
        return group ?: synchronized(this) {
            group ?: st_gid.toString().also { group = it }
        } ?: ""
    }

    fun permissions(): java.util.Set<FilePermission> {
        val bits = st_mode and UnixConstants.S_IAMB
        val perms = java.util.HashSet<FilePermission>()
    ...
    fun getDeviceString(): String = "Device: `${st_dev.toString(16)}`"
    ...
    fun getOwnerString(): String = "Owner: `${owner() ?: "unknown"}`"

    fun getGroupString(): String = "Group: `${group() ?: "unknown"}`"

    fun getPermissionString(): String = "Permissions: `${FilePermissions.toString(permissions())}`"

    fun getAccessTimeString(): String = "Access Time: `${lastAccessTime() ?: "unknown"}`"

    fun getModifiedTimeString(): String = "Modified Time: `${lastModifiedTime() ?: "unknown"}`"

    fun getChangeTimeString(): String = "Change Time: `${lastChangeTime() ?: "unknown"}`"
