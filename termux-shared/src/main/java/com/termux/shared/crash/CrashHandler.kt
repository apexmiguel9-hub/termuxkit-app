...
class CrashHandler private constructor(
    ...
    fun logCrash(@NonNull thread: Thread, @NonNull throwable: Throwable) {
        if (!mCrashHandlerClient.onPreLogCrash(context, thread, throwable)) {
            logCrashToFile(context, mCrashHandlerClient, thread, throwable)
            mCrashHandlerClient.onPostLogCrash(context, thread, throwable)
        }
    }
