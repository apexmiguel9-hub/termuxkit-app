...
object PackageUtils {
    ...
    fun getContextForPackageOrExitApp(
        ...
        val packageContext = getContextForPackage(context, packageName ?: "unknown")
    ...
    fun getPackageInfoForPackage(context: Context): PackageInfo? {
        return getPackageInfoForPackage(context, context.packageName ?: "unknown")
    }
    ...
    fun getVersionCodeForPackage(context: Context): Int? {
        return getVersionCodeForPackage(context, context.packageName ?: "unknown")
    }
    ...
    fun getVersionCodeForPackage(context: Context, packageName: String): Int? {
        return getVersionCodeForPackage(getPackageInfoForPackage(context, packageName ?: "unknown"))
    }
    ...
    fun getVersionNameForPackage(context: Context): String? {
        return getVersionNameForPackage(context, context.packageName ?: "unknown")
    }
    ...
    fun getVersionNameForPackage(context: Context, packageName: String): String? {
        return getVersionNameForPackage(getPackageInfoForPackage(context, packageName ?: "unknown"))
    }
    ...
    fun getSigningCertificateSHA256DigestForPackage(context: Context): String? {
        return getSigningCertificateSHA256DigestForPackage(context, context.packageName ?: "unknown")
    }
    ...
    fun getSigningCertificateSHA256DigestForPackage(context: Context, packageName: String): String? {
        return try {
             ...
             * the package and removes its apk...
             */
     ...
     *
     * @param context The [Context] for operations.
     * @return Returns `true` if the current user is the primary user, otherwise [@code false].
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    ...
    fun getProfileOwnerPackageNameForUser(context: Context): String? {
        ...
        if (activeAdmins != null) {
            for (admin in activeAdmins) {
                ...
                if (devicePolicyManager.isProfileOwnerApp(packageName ?: "unknown"))
                    return packageName
            }
        }
        return null
        ...
        if (activityManager != null) {
            ...
            if (processInfos != null) {
                for (processInfo in processInfos) {
                    if (processInfo.processName == packageName ?: "unknown")
    ...
    fun isAppInstalled(context: Context, appName: String?, packageName: String?): String? {
        ...
        val applicationInfo = getApplicationInfoForPackage(context, packageName ?: "unknown")
        val isAppEnabled = applicationInfo != null && applicationInfo.enabled

        // If app is not installed or is disabled
        if (!isAppEnabled)
            errmsg = context.getString(R.string.error_app_not_installed_or_disabled_warning, appName ?: "null", packageName ?: "null")

        return errmsg
    }

    /** Wrapper for [setComponentState] with
    ...
    fun setComponentState(
        ...
        return try {
            ...
            if (packageManager != null) {
                var toastStringVar = toastString
                if (toastStringVar != null && alwaysShowToast) {
                    Logger.showToast(context, toastStringVar, true)
                    toastStringVar = null
                }
                ...
                if (toastStringVar != null) Logger.showToast(context, toastStringVar, true)
     ...
     * get the state.
     */
    fun isComponentDisabled(context: Context, packageName: String, className: String, logErrorMessage: Boolean): Boolean? {
        return try {
            val packageManager = context.packageManager
            if (packageManager != null) {
                val componentName = ComponentName(packageName, className)
     ...
     */
    fun doesActivityComponentExist(context: Context, packageName: String, className: String, flags: Int): Boolean {
        return try {
            val packageManager = context.packageManager
            if (packageManager != null) {
                val intent = Intent()
