...
object UserUtils {
    ...
    fun getNameForUidFromLibcore(uid: Int): String? {
        ...
        return try {
            ...
            val clazz = Class.forName(libcoreClassName)
            ...
            val superClazz = os.javaClass.superclass  // libcore.io.ForwardingOs
            if (superClazz == null) {
            ...
            try {
                val getpwuidMethod = ReflectionUtils.getDeclaredMethod(superClazz, "getpwuid", Int::class.javaPrimitiveType as Class<*>)
                    ?: return null
                structPasswd = ReflectionUtils.invokeMethod(getpwuidMethod, os, uid).value
            } catch (e: Exception) {
                Logger.logStackTraceWithMessage(LOG_TAG, "Failed to invoke getpwuid() method of ${superClazz.name} class", e)
                return null
            }
            ...
            var passwdClazz: Class<*>? = null
            try {
                passwdClazz = structPasswd.javaClass
                ReflectionUtils.invokeField(passwdClazz, "pw_name", structPasswd).value as? String
            } catch (e: Exception) {
                // ClassCastException may be thrown
                Logger.logStackTraceWithMessage(LOG_TAG, "Failed to get \"pw_name\" field value for ${passwdClazz?.name} class", e)
                null
            }
        } catch (e: Exception) {
