 ...
 * default value is with reflection after bypassing hidden api restrictions since [FEATURE_FLAGS_CLASS]
 * is annotated as `@hide`.
 */
object FeatureFlagUtils {

    enum class FeatureFlagValue(val displayName: String) {
        ...
        /** Get the display name for the feature flag value */
    ...
    fun getAllFeatureFlags(): Map<String, String>? {
        ...
        return try {
            @SuppressLint("PrivateApi") val clazz = Class.forName(FEATURE_FLAGS_CLASS)
            ...
            getAllFeatureFlagsMethod.invoke(null) as? Map<String, String>
        } catch (e: Exception) {
    ...
    fun getFeatureFlagValueString(@NonNull context: Context, @NonNull feature: String): FeatureFlagValue {
        ...
        if (featureFlagValue == null) {
            ...
            return if (featureFlagValue) FeatureFlagValue.TRUE else FeatureFlagValue.FALSE
        }
    }
    ...
    fun isFeatureEnabled(@NonNull context: Context, @NonNull feature: String): Boolean? {
        ...
        return try {
            ...
            isFeatureEnabledMethod.invoke(null, context, feature) as? Boolean
        } catch (e: Exception) {
            // ClassCastException may be thrown
